...
const NFLMatchListPage = () => {
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      const competition = new URLSearchParams(location.search).get('competition');  // Get the competition ID from the query params
      const token = process.env.REACT_APP_NFL_API_TOKEN;
      const apiBaseUrls = process.env.REACT_APP_NFL_API_BASE_URL;
    ...
    fetchMatches();
  }, [location.search]);  // Watch location.search instead of location to ensure re-fetch when query params change

  const handleViewPrediction = (matchId) => {
    navigate(`/nfl/match-prediction?match=${matchId}`);
  };
...
export default NFLMatchListPage;
